...
public class MainScreen {
    ...
    public void buildMainScreen() {
        ...
        for (int r = 0; r < 9; r+=3) {
            ...
            for (int c = 0; c < 9; c+=3) {
                var endCol = c + 2;
                var spaces = getSpacesFromSector(boardService.getSpaces(), c,
                        endCol, r, endRow);
    ...
    private JPanel generateSection(List<Space> spaces){
        List<NumberText> fields =
                new ArrayList<>(spaces.stream().map(NumberText::new).toList());
    ...
    private void addFinishGameButton(JPanel mainPanel) {
         finishGameButton = new FinishGameButton(e -> {
            if (boardService.gameIsFinished()) {
                JOptionPane.showMessageDialog(null, "Parabéns você concluiu o" +
                                                    " Jogo");
                resetButton.setEnabled(false);
                checkGameStatusButton.setEnabled(false);
                finishGameButton.setEnabled(false);
            } else {
                var message = "Seu Jogo tem algum erro, ajuste e tente " +
                              "novamente";
                JOptionPane.showMessageDialog(null, message);
            }
        });
